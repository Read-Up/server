...
class CookieProviderTest {
	...
	void testGenerateAccessTokenCookie() {
		...
		ResponseCookie accessTokenCookie = cookieProvider.generateAccessTokenCookie(accessToken, expiresMS);
		...
		assertEquals(Duration.ofMillis(expiresMS), accessTokenCookie.getMaxAge());
	}
	...
	void testGenerateRefreshTokenCookie() {
		...
		ResponseCookie refreshTokenCookie = cookieProvider.generateRefreshTokenCookie(refreshToken, expiresMS);
		...
		assertEquals(Duration.ofMillis(expiresMS), refreshTokenCookie.getMaxAge());
	}

	@Test
	@DisplayName("액세스 삭제 쿠키 생성하기")
	void testGenerateDeletedAccessTokenCookie() {
		ResponseCookie refreshTokenCookie = cookieProvider.generateDeletedAccessTokenCookie();

		assertEquals("access_token", refreshTokenCookie.getName());
		assertEquals("", refreshTokenCookie.getValue());
		assertEquals("/", refreshTokenCookie.getPath());
		assertEquals(Duration.ofMillis(0), refreshTokenCookie.getMaxAge());
	}

	@Test
	@DisplayName("리프레쉬 삭제 쿠키 생성하기")
	void testGenerateDeletedRefreshTokenCookie() {
		ResponseCookie refreshTokenCookie = cookieProvider.generateDeletedRefreshTokenCookie();
