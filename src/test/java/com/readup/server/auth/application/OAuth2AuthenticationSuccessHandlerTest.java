...
class OAuth2AuthenticationSuccessHandlerTest {
	...
	void testOnAuthenticationSuccess_NewUser() throws IOException {
		try (MockedStatic<SecurityContextHolder> securityContextHolderMock = mockStatic(SecurityContextHolder.class);
			...
			ResponseCookie accessTokenCookie = ResponseCookie.from("accessToken", authTokens.accessToken()).build();
			ResponseCookie refreshTokenCookie = ResponseCookie.from("refreshToken", authTokens.refreshToken()).build();
