...
class LogoutAuthFilterTest {
	...
	void setUp() {
		...
		Cookie[] cookies = new Cookie[] {new Cookie("access_token", "test-token"),
			new Cookie("refresh_token", "test-token")};
		...
		ResponseCookie deletedAccessCookie = ResponseCookie.from("access_token", "").maxAge(0).build();
		ResponseCookie deletedRefreshCookie = ResponseCookie.from("refresh_token", "").maxAge(0).build();
	...
	void shouldHandleEmptyRefreshToken() {
		...
		Exception exception = assertThrows(ServiceException.class,
			() -> logoutAuthFilter.doFilterInternal(testRequest, response, filterChain));
	...
	void shouldRedirectAfterLogout() throws ServletException, IOException {
		...
		request.setServletPath("/api/public/logout");
		request.setMethod("GET");
		request.setParameter("redirect", redirectUri);
