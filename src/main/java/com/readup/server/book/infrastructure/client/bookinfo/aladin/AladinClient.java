...
public class AladinClient implements BookInfoClientFacade {
	...
	public BookInfoVO getBookInfo(String isbn) {
		...
		AladinGetBookResponse aladinGetBookResponse = parseAladinResponse(rawResponse);
	...
	private AladinGetBookResponse parseAladinResponse(String rawResponse) {
		String fixedJson = rawResponse.replace("\\'", "'");
		try {
			return objectMapper.readValue(fixedJson, AladinGetBookResponse.class);
		} catch (Exception e) {
