...
public record GetUserQuizSetResponse(Long userQuizSetId, Long lastQuizId, Boolean isEvaluated, Boolean isDone) {
	...
	public static GetUserQuizSetResponse from(UserQuizSet userQuizSet) {
		return new GetUserQuizSetResponse(userQuizSet.getId(), userQuizSet.getLastQuizId(),
			userQuizSet.getIsEvaluated(), userQuizSet.getIsDone());
	}
}
